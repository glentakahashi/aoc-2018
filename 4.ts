let data = `[1518-10-14 00:05] falls asleep
...
for(let i = 0; i < rows.length; i++) {
    ...
    if(row[2] === 'falls') {
        start = parseInt(row[1].split(':')[1]);
    } else {
        let end = parseInt(row[1].split(':')[1]);
        guardTimes[guard].count += end - start;
        for(let j = start; j < end; j ++) {
            guardTimes[guard].arr[j] ++;
        }
    }
}

let ks = Object.keys(guardTimes);
let cg = null;
let currentMax = -1;
let maxGuardMinuteGuard = null;
let maxGuardMinuteIndex = -1;
let maxGuardMinuteCount = -2;
for(let  i= 0;i<ks.length;i++) {
    for(let j = 0; j < 60; j++) {
        if(guardTimes[ks[i]].arr[j] > maxGuardMinuteCount) {
            maxGuardMinuteGuard = ks[i];
            maxGuardMinuteIndex = j;
            maxGuardMinuteCount = guardTimes[ks[i]].arr[j];
        }
    }

    if(guardTimes[ks[i]].count >  currentMax) {
        cg = ks[i];
        currentMax = guardTimes[ks[i]].count;
    }
}
console.log(cg);
console.log(maxGuardMinuteGuard, maxGuardMinuteIndex, maxGuardMinuteCount);

let ci = 0;
let cm = 0;
for(let i = 0; i < 60; i++) {
    if(guardTimes[cg].arr[i] > cm) {
        cm = guardTimes[cg].arr[i];
        ci = i;
    }
}
